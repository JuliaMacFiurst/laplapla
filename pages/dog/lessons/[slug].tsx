...
export default function LessonPlayer() {
  ...
  useEffect(() => {
    ...
    if (frank === lastFrankRef.current) return; // Защита от лишнего запроса
    lastFrankRef.current = frank;

    let cancelled = false;
    async function fetchFibiSpeech() {
      setFibiSpeech(""); // сбросить на время загрузки
      try {
        const apiKey = process.env.GEMINI_API_KEY || "";
        if (!apiKey) {
          setFibiSpeech("Фиби молчит (нет API ключа)");
          return;
        }
        ...
        setFibiSpeech("Фиби задумалась... (ошибка генерации)");
      }
    }
    fetchFibiSpeech();
    return () => {
      ...
      {lesson ? (
        <div>
          ...
          <div className="lesson-controls">
            ...
            >
              {currentStepIndex < lesson.steps.length - 1 ? "Следующий шаг 🐾" : "Готово!✅"}
            </button>
              ...
              <div style={{ marginTop: "10px", textAlign: "center" }}>
                <button
                  ...
                  onClick={() => {
                    ...
                    if (newSketch) {
                      setShowColorizer(false); // временно скрываем
                      setTimeout(() => {
                        setShowColorizer(true); // заново отрисовываем с новым изображением
                      }, 10);
                    }
                  }}
                >
            ...
            <div className="lesson-step-counter">
              {lesson && currentStepIndex >= 0 && (
                <p>{currentStepIndex + 1} шаг из {lesson.steps.length}</p>
              )}
            </div>
          </div>
            ...
            <div className="lesson-frank" style={{ display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
              <div className="lesson-frank-tip" style={{ marginTop: "270px" }}>
                {currentStepIndex >= 0 && lesson?.steps[currentStepIndex]?.frank ? (
                  ...
                  <>
                    <p>Приготовь мышку, стилус, тачпад, или просто листик и карандаш, и жми на Старт!</p>
                    <DogImage name="frank" pose={frankPose} speech="Привет! Я Фрэнк и я помогу тебе научиться рисовать!" size={220} />
                  </>
                )}
              </div>
            </div>
          ...
          <div className="lesson-toolbar">
            ...
            <button
              ...
              onClick={() => {
                if (confirm("Уверены, что хотите очистить холст? Это действие нельзя отменить.")) {
                  const canvas = drawingCanvasRef.current;
                  const ctx = canvas?.getContext("2d");
                  if (canvas && ctx) {
                    const fadeOut = () => {
