...
export const getServerSideProps: GetServerSideProps<Props> = async (context) => {
  ...
  const { title, groupedStories, hasAnyStories } = await loadSeoEntityPageData("river", slug, lang);
  if (!hasAnyStories) {
    return { notFound: true };
  }
...
export default function RiverSeoPage({
  title,
  groupedStories,
  lang,
}: InferGetServerSidePropsType<typeof getServerSideProps>) {
  return <SeoEntityPage entityType="river" title={title} groupedStories={groupedStories} lang={lang} />;
}
