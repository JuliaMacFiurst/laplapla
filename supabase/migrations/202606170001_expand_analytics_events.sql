alter table public.analytics_events
  ...
  add column if not exists readable_title text,
  ...
  add column if not exists export_surface text,
  add column if not exists project_id text,
  add column if not exists scroll_percent numeric;

update public.analytics_events
set
  properties = coalesce(properties, '{}'::jsonb) || jsonb_strip_nulls(jsonb_build_object(
    'section', metadata->>'section',
    'content_type', coalesce(entity_type, metadata->>'content_type'),
    ...
    'page_title', coalesce(entity_title, metadata->>'page_title'),
    'readable_title', coalesce(entity_title, metadata->>'readable_title'),
  ...
  content_type = coalesce(content_type, entity_type, metadata->>'content_type'),
  ...
  page_title = coalesce(page_title, entity_title, metadata->>'page_title'),
  readable_title = coalesce(readable_title, entity_title, metadata->>'readable_title'),
